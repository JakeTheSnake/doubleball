...
GameCreator.effects.FadeOut = function(runtimeObj) {
    ...
    this.image = runtimeObj.attributes.image;
...
GameCreator.effects.Shrink.prototype.draw = function(context) {
    if (this.width >= 0 && this.height >= 0) {
        context.drawImage(this.runtimeObj.attributes.image, this.x, this.y, this.width, this.height);
        return true;
    }
    return false;
}
...
GameCreator.effects.RiseAndFade.prototype.draw = function(context) {
    if (this.currentAlpha >= 0) {
        context.globalAlpha = this.currentAlpha;
        context.drawImage(this.runtimeObj.attributes.image, this.runtimeObj.attributes.x, this.y, this.runtimeObj.attributes.width, this.runtimeObj.attributes.height);
        context.globalAlpha = 1.0;
        return true;
    }
    return false;
